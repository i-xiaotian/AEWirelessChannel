    ...
    public boolean onBlockActivated(final World worldIn, @Nonnull final BlockPos pos, @Nonnull final IBlockState state, @Nonnull final EntityPlayer playerIn, final EnumHand hand, final EnumFacing facing, final float hitX, final float hitY, final float hitZ) {
        ...
        if (item == Items.ITEM_WIRELESS_KEY_CARD) {
            ...
            if (!(tileEntity instanceof TileWirelessOutputBus)) {
                playerIn.sendMessage(new TextComponentString(I18n.format("message.aewirelesschannel.bount_type_error")));
                return true;
            }
            final TileEntity inputBus = worldIn.getTileEntity(pos);
            if (!(inputBus instanceof TileWirelessInputBus tileWirelessInputBus)) {
                playerIn.sendMessage(new TextComponentString(I18n.format("message.aewirelesschannel.bount_type_error_input")));
                return true;
            }
            ...
            currentConnection.setTargetX(boundX);
            currentConnection.setTargetY(boundY);
            currentConnection.setTargetZ(boundZ);
